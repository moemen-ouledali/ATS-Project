...
const UserManagement = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('https://ats-project-1.onrender.com/auth/users');
  ...
  const handleChangeRole = async () => {
    try {
      await axios.put(`https://ats-project-1.onrender.com/auth/user/${selectedUser._id}/role`, { role: newRole });
  ...
  const handleCreateManager = async () => {
    try {
      const response = await axios.post('https://ats-project-1.onrender.com/auth/register', {
        role: 'Manager',
