  ...
  const fetchApplicants = useCallback(async () => {
    try {
      const [applicantsResponse, jobResponse] = await Promise.all([
        axios.get(`https://ats-project.onrender.com/api/applications/for-job/${jobId}`),
        axios.get(`https://ats-project.onrender.com/api/jobs/${jobId}`)
      ]);
          ...
          {selectedApplicant && (
            <Modal show onHide={closeApplicationDetails}>
              ...
              <Modal.Body>
                ...
                <Typography variant="body2" color="textSecondary" component="p"><strong>Resume:</strong> <a href={`https://ats-project.onrender.com/${selectedApplicant.resumePath}`} target="_blank" rel="noopener noreferrer">View Resume</a></Typography>
