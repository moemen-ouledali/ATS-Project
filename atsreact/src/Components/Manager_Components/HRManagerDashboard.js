...
const HRManagerDashboard = () => {
  ...
  const fetchJobListings = async () => {
    setLoading(true);
    try {
      const response = await axios.get('https://ats-project.onrender.com/api/jobs');
  ...
  const deleteJobListing = async () => {
    try {
      await axios.delete(`https://ats-project.onrender.com/api/jobs/${deleteJobId}`);
      fetchJobListings();
  ...
  const handleSave = async (id, updatedListing) => {
    try {
      await axios.put(`https://ats-project.onrender.com/api/jobs/${id}`, updatedListing);
      fetchJobListings();
      setEditingId(null);
